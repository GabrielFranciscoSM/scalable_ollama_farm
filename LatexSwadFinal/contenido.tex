...
%     tabsize=2,
%     texcl=true]{/home/manuadmin/P5/P5-ab/DockerFileAb}
...
%     firstline=49,
%     lastline=59,
%     texcl=true]{/home/manuadmin/P5/logdLocust.txt}
...
%     firstline=14,
%     lastline=17,
%     texcl=true]{/home/manuadmin/P5/P5-granjaWordpress/P4-manuel100-nginx/manuel100-nginx-ssl.conf}
...
%         \centering
%         \includegraphics[width=\textwidth]{Images/imagen_20.png}
%         \caption{Configuramos la cuenta de administración.}
%     \end{subfigure}
%     \hfill
%     \begin{subfigure}[H]{0.49\textwidth}
%         \centering
%         \includegraphics[width=\textwidth]{Images/imagen_21.png}
%         \caption{Accedemos exitosamente al `home'.}
%     \end{subfigure}
% \end{figure}
...
% Este trabajo presenta el diseño e implementación de una granja local de modelos de lenguaje basada en contenedores. La solución busca ofrecer un entorno reproducible, escalable y orientado a la disponibilidad para servir modelos LLM en equipos modestos, priorizando la estabilidad frente al rendimiento máximo.

% \section{Antecedentes y conceptos previos}
% En esta sección se introducen los conceptos necesarios para comprender el proyecto: inferencia local con Ollama, enrutamiento mediante LiteLLM, proxy inverso con Nginx y almacenamiento temporal con Redis. También se revisan ideas como escalabilidad horizontal, balanceo de carga y caché semántica.

% \section{Objetivos}
% \subsection{Objetivo general}
% Diseñar y validar una infraestructura local para servir modelos LLM mediante Docker Compose, con posibilidad de escalado y mejora de disponibilidad.

% \subsection{Objetivos específicos}
% \begin{itemize}
%     \item Desplegar uno o varios nodos de inferencia con Ollama.
%     \item Definir un punto único de acceso mediante proxy o gateway.
%     \item Integrar Redis como soporte para caché semántica.
%     \item Medir latencia, throughput y comportamiento con y sin caché.
%     \item Documentar la arquitectura y las decisiones técnicas adoptadas.
% \end{itemize}

% \section{Arquitectura propuesta}
% Se describe la solución formada por cliente, proxy de entrada, nodos Ollama, Redis y scripts de despliegue. También se explica el flujo de una petición desde el cliente hasta la generación de la respuesta.

% \section{Desarrollo y análisis}
% \subsection{Despliegue con Docker Compose}
% Aquí se detallan los servicios definidos en \texttt{docker-compose.yml}, las redes, los volúmenes y las variables de entorno.

% \subsection{Configuración de los componentes}
% Se explica la configuración de Ollama, LiteLLM, Nginx y Redis, así como los scripts auxiliares de inicialización.

% \subsection{Estrategia de caché}
% Se describe el mecanismo utilizado para almacenar y recuperar respuestas frecuentes, incluyendo claves, TTL y criterios de coincidencia.

% \section{Pruebas y resultados}
% Se presentan los experimentos realizados, los casos de prueba y las métricas obtenidas. Se comparan los escenarios con y sin caché, y se analizan los resultados.

% \section{Conclusiones}
% Se resumen los logros alcanzados, las limitaciones del sistema y posibles mejoras futuras.
...
En este trabajo abordamos estos problemas construyendo una granja de modelos de IA que combina tres pilares fundamentales: \textit{replicación de instancias de inferencia}, \textit{balanceo de carga} y \textit{caché semántico}. El sistema se despliega mediante Docker Compose y está compuesto por seis servicios: dos instancias de Ollama para inferencia con el modelo \texttt{qwen3.5:0.8b}, una instancia de Ollama dedicada a embeddings con \texttt{nomic-embed-text}, un gateway LiteLLM que actúa como punto de enrutamiento y gestor de caché, Redis Stack como almacén del caché semántico vectorial, y Nginx como proxy inverso de entrada.

La novedad principal del trabajo reside en el uso de caché semántico: en lugar de limitarnos a servir respuestas idénticas desde caché (caché exacto), empleamos Redis con búsqueda por similitud coseno sobre embeddings para detectar preguntas parafraseadas y devolver la respuesta almacenada. Los resultados muestran una aceleración de 191.5$\times$ en latencia para aciertos de caché (31 ms frente a 5929 ms) y una distribución de carga equilibrada entre las instancias de inferencia.
...
Para comprender el diseño y el funcionamiento de la granja de modelos propuesta, es necesario introducir las tecnologías y conceptos que sirven de base a la infraestructura. A continuación se detallan las herramientas de contenerización, inferencia, enrutamiento, almacenamiento en caché y análisis de rendimiento utilizadas en este proyecto.
...
La similitud coseno~\cite{cosineSim} entre dos vectores $\vec{a}$ y $\vec{b}$ se define como:
\begin{equation}
    \text{sim}(\vec{a}, \vec{b}) = \frac{\vec{a} \cdot \vec{b}}{\|\vec{a}\| \cdot \|\vec{b}\|} \in [-1, 1]
\end{equation}

Dos vectores idénticos tienen similitud~1, vectores ortogonales~0, y vectores opuestos~{-1}. En la práctica, con embeddings normalizados, el umbral 0.85 captura paráfrasis cercanas.%\footnote{El producto del numerador es vectorial.}
...
\subsection{Locust y Python}

Locust~\cite{locust} se trata de una herramienta que permite realizar pruebas de carga realistas para contextos con usuarios, esto con la función de encontrar cuellos de botellas, validar la capacidad del sistema, ... Pero en nuestro contexto lo empleamos para realizar las pruebas de funcionamiento. Es de código abierto y esta basado en el lenguaje Python, lo que permite aumentar la complejidad y la versatilidad de las pruebas definiendo \emph{tasks} en un archivo \emph{locustfile.py}. Puede combinarse con Docker en lugar de emplear la librería \emph{subprocess}.

También puede emplearse y empleamos un código alternativo con funcionalidades muy útiles en Python con el que ejecutamos otra prueba, pero por supuesto, el código es más difícil de comprender y modificar, y este no podría extenderse tan fácilmente a realizar pruebas de cargas.
...
La solución diseñada se basa en una estructura modular donde cada componente desempeña una función específica para garantizar el balanceo de carga y el almacenamiento eficiente de las respuestas. En esta sección se presenta el diagrama de bloques global del sistema y se detalla la función que cumple cada uno de los servicios desplegados.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (litellm) -- (ollamaEmb) node[midway, right, yshift=2.5cm, font=\footnotesize] {embed};
...
\textbf{Redis Stack} actúa como almacén del caché semántico, siendo una extensión de Redis que facilita tratar datos complejos. A través de su módulo \texttt{RedisSemanticCache}, LiteLLM crea automáticamente un índice vectorial en Redis para almacenar las respuestas generadas. Ante búsquedas de similitud, Redis devuelve la respuesta más cercana si supera el umbral anteriormente mencionado.
...
Una vez definida la arquitectura, la puesta en marcha del sistema se materializa mediante la configuración detallada de los servicios y sus directivas de red. A continuación se presentan los archivos de configuración y los scripts de inicialización que permiten automatizar y coordinar el arranque de toda la granja de modelos.
...
\begin{lstlisting}[    language=bash,
    ...
    numberstyle=\tiny\color{gray},
    tabsize=2, 
 caption=Script de inicialización de Ollama (inferencia), label=lst:ollamainit]
#!/bin/sh
set -e
...
\begin{lstlisting}[language=config3, caption=configv2.yaml --- Configuración de LiteLLM, label=lst:litellmconfig, basicstyle=\ttfamily\small, breaklines=true, frame=single, numbers=none]
model_list:
  - model_name: qwen3
    litellm_params:
...
Con el objetivo de cuantificar las ventajas de la caché semántica y la equidad del balanceo de carga, se ha sometido la infraestructura a diversas pruebas experimentales. En esta sección se describe la metodología de medición empleada, la estructura de los grupos de prueba y se analizan detalladamente los resultados obtenidos tanto de manera individual como bajo escenarios de concurrencia.

\subsection{Metodología de evaluación}

Para evaluar el rendimiento del sistema se desarrolló el \textit{script} de pruebas \texttt{test\_litellm.py} en Python. Este ejecuta 89 peticiones organizadas en siete grupos experimentales, cada uno diseñado para verificar un aspecto concreto del comportamiento del caché semántico y del balanceo de carga. De forma análoga, empleamos las peticiones para ejecutar Locust y estudiar los tiempos de respuesta.

La detección de aciertos de caché (HIT) y fallos (MISS) se realiza mediante el análisis de los registros (\textit{logs}) del contenedor de LiteLLM. LiteLLM escribe la cadena \texttt{"Cache Hit!"} en su salida estándar de forma síncrona antes de enviar la respuesta HTTP. El \textit{script} toma una instantánea de los logs antes y después de cada petición; si el contador de ocurrencias de \texttt{"Cache Hit!"} se incrementa, la petición se clasifica como HIT, y como MISS en caso contrario. La instancia de Ollama que atendió cada petición se determina contando las líneas que contienen la URL del \textit{api\_base} correspondiente (\texttt{http://ollama-1:11434} o \texttt{http://ollama-2:11434}). Locust, por otra parte, tras haber configurado en su archivo la fracción correspondiente a cada tarea (\emph{@task}); una por grupo; recoge las métricas correspondientes a cada grupo y podremos consultarlas, ya sea a través de la interfaz o mediante archivos `.csv'.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[H]{0.50\textwidth}
        ...
        \label{fig:balanceo}
    \end{subfigure}
    \caption{Gráficos, primera prueba.}
\end{figure}%

\newpage 
\subsubsection{Locust}

En la siguiente tabla, vemos el número de peticiones de cada grupo y sus tiempos mínimos, máximos y medios, e incluso el tamaño medio de la respuesta.

\begin{table}[H]
\centering
\caption{Resultados agregados por grupo experimental en Locust.}
    \label{tab:resultadosL}
\small
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrrrr}
  \toprule
  \textbf{Grupo} & \textbf{Peticiones} & \textbf{Mediana RT} & \textbf{Media RT }& \textbf{Mín. RT} & \textbf{Máx. RT} & \textbf{Tamaño medio} \\
  \midrule
  Duplicados exactos & 129 & 310 & 3780 & 45 & 52933 & 308 \\
  Balanceo de carga  & 12 & 28000 & 25615 & 12583 & 44469 & 281 \\
  Paráfrasis Geografía & 24 & 250 & 9864 & 56 & 52635 & 304 \\
  Paráfrasis Historia & 28 & 880 & 8357 & 59 & 37526 & 300 \\
  Paráfrasis Ciencia & 40 & 250 & 7619 & 57 & 47966 & 303 \\
  Relacionados distintos & 23 & 1400 & 13803 & 60 & 44017 & 298 \\
  No relacionados & 15 & 410 & 8302 & 60 & 59903 & 305 \\
  \bottomrule
  \end{tabular*}
\end{table}

Donde RT abrevia \emph{Request Time}, los tiempos se miden en milisegundos y el tamaño en bytes. Se han obviado las columnas de fallos, que estaban a cero, y los porcentiles los aportamos a modo de gráfica a continuación.

\begin{figure}[H]
  \begin{center}
    \includegraphics[width=\textwidth]{Images/graf_perc.png}
  \end{center}
  \caption{Tiempos según percentiles de grupos experimentales en una gráfica.}
\end{figure}

\subsection{Análisis de los resultados}

\textbf{Caché exacto (100\,\% de acierto).} Las 10 preguntas repetidas de forma idéntica en el grupo de duplicados exactos obtuvieron HIT en su segunda aparición sin excepción. Esto confirma que el flujo básico de almacenamiento y recuperación del caché funciona correctamente. Respecto a sus tiempos, aproximadamente a partir del percentil noventa, donde vemos métricas altas, coincide la proporción de las peticiones por primera vez (10) respecto al total del grupo (129), que están formadas por la mismas diez peticiones repetidas, y sus tiempos de respuesta se reducen con el caché. 
...
La correlación entre un aumento de las métricas temporales y el número de MISS debería poder apreciarse si todas las peticiones de paráfrasis fueran distintas, pero en este caso (Locust) se repiten y no hay mecanismo para hacer un uso equitativo de estas, y como el número de peticiones realizadas supera el número de peticiones distintas es obvio que se han repetido. Los últimos percentiles (probablemente correspondientes a las primeras veces de cada petición) nos muestran que la dificultad de los temas de algunos grupos de paráfrasis es distinta, ya sea por las preguntas concretas o la configuración del modelo en sí.
...
\textbf{Latencia.} La diferencia es drástica: las peticiones resueltas desde caché promedian 31~ms, frente a los 5929~ms que requiere la inferencia completa en CPU con el modelo \texttt{qwen3.5:0.8b}. El factor de aceleración es de 191.5$\times$ (mediana de 31~ms frente a 5925~ms). La latencia de caché está dominada por la generación del embedding de la pregunta entrante y la consulta al índice vectorial de Redis; la latencia de inferencia refleja el coste computacional de generar la respuesta token a token en CPU. Con Locust la proporción se mantiene considerando los tiempos de la mediana y el porcentil noventa y ocho, puesto que los tiempos mínimos se alejan de la proporción anterior por la carga, que ralentiza al sistema y parece aumentar casi diez veces el resto de métricas.

\textbf{Balanceo de carga.} La estrategia \texttt{simple-shuffle} produjo una distribución prácticamente equitativa: 45 peticiones a ollama-1 (51\,\%) y 44 a ollama-2 (49\,\%). La pequeña asimetría es esperable en un reparto aleatorio con un número finito de muestras. Como solicitamos que se emplee el caché en este grupo, es comprensible que sus tiempos sean altos pero con poca variación, ya que la petición es idéntica cada vez.

\newpage
\section{Conclusiones}

El diseño e implementación de esta granja local de modelos de IA ha permitido constatar la viabilidad de servir modelos de lenguaje en equipos modestos mediante mecanismos de optimización. A modo de balance final, en esta sección se evalúa el cumplimiento de los objetivos propuestos, se discuten las limitaciones encontradas y se definen las principales líneas de trabajo futuro.
...
\begin{itemize}
    \item El caché semántico proporciona una aceleración de 191.5$\times$ en latencia para preguntas repetidas o parafraseadas (31~ms frente a 5929~ms), eliminando la necesidad de ejecutar inferencia redundante.
    ...
    \item El balanceo de carga distribuye las peticiones de forma equitativa (51\,\%/49\,\%) mediante la estrategia \texttt{simple\allowbreak-shuffle} sin lógica compleja.
    
    \item El sistema puede mantener un funcionamiento y velocidad correctos de aproximadamente dos peticiones por segundo con cargas moderadas (aunque repetidas) con tan solo dos instancias.
...
\begin{enumerate}
    ...
    \item \textbf{Nuevo test de carga}: podemos rehacer/extender el \emph{locustfile.py} para; en un entorno con más instancias; ejecutar una nueva prueba de carga, con lo que podríamos hacernos una idea de los límites del conjunto ampliado e identificar otros aspectos a mejorar o necesidades futuras, de los que tan solo podemos atisbar algo ahora.
    
    %No comprendo la finalidad de varios nodos, si no es necesario ni viable para nada que se nos ocurra sería mejor quitarlo, no estará en la presentación hasta confirmarlo
...
Además, existen posibles mejoras en el campo de red:

\begin{enumerate}
    \item \textbf{Reglas}: se pueden añadir medidas para eliminar el tráfico innecesario (IPTABLES, firewall, configuración de router...).
    
    \item \textbf{Certificado SSL}: podríamos dotar a Nginx con este y así gozar de conexión cifrada, alterando su configuración.
    
    \item \textbf{Optimización}: para mejorar la eficiencia, se pueden tener en cuenta características de la red como tamaño de paquetes y ancho de banda, y tomar medidas que mejoren el rendimiento medio. 
\end{enumerate}
